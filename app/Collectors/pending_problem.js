...
const SemesterSubjects = () => {
  ...
  const fetchSemesters = async () => {
    try {
      ...
      if (jsonValue) {
        ...
        const values1 = { sp: 814, user_id: userData.result.auto_id, typ: 'Pending' };
  ...
  const handleDropdownChange1 = (itemValue) => {
   
    setF1(itemValue)
  };
  const handleDropdownChange2 = (itemValue) => {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.semesterContainer}>
        ...
        <Text style={styles.semesterText}>Cabashooyinka Pending ah</Text>
      </View>
          ...
          {expandedSubject === subject.id && (
            <View style={styles.expandedContainer}>
              ...
              <View style={styles.tableRow}>
                ...
                <Text style={styles.rowValue}>{subject.Zone || 'No Data'}</Text>
              </View>
              <View style={styles.tableRow}>
                <Text style={styles.rowLabel}>Deriska:</Text>
                <Text style={styles.rowValue}>{subject.Deriska || 'No Data'}</Text>
              </View>
              <View style={styles.tableRow}>
                <Text style={styles.rowLabel}>Cabashada:</Text>
                <Text style={styles.rowValue}>{subject.Problem || 'No Data'}</Text>
              </View>
              <View style={styles.tableRow}>
                ...
                <Text style={styles.rowValue}>{subject.Cabashada || 'No Data'}</Text>
                  ...
                  <View style={styles.dropdownContainer}>
                    <Picker
                      selectedValue={f2}
                      onValueChange={(itemValue) =>
                        handleDropdownChange2(itemValue)
                      }
                    >
                      <Picker.Item label="Dooro farsamo yaqaan 2" value={0} />
                      {farsamo.map((option) => ( 
                        <Picker.Item key={option.auto_id} label={option.name} value={option.auto_id} />
                      ))}
                    </Picker>
...
const styles = StyleSheet.create({
  ...
  dropdownContainer: {
    height: 50,                  // Height of the dropdown container
    width: '100%',                // Width of the dropdown container (adjust as needed)
    borderColor: 'blue',         // Light gray border color
    borderWidth: 1,              // Border width
    borderRadius: 5,             // Rounded corners
    backgroundColor: '#f9f9f9',  // Background color of the dropdown
    justifyContent: 'center',    // Centers the content vertically
    paddingLeft: 10,             // Padding on the left for spacing
    marginBottom: 15,            // Space below the dropdown
  },
  
  dropdownText: {
    fontSize: 16,                // Font size for the dropdown text
    color: '#333',               // Font color (dark gray for text)
